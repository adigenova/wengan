...
sub create_jobs{
  ...
  if(defined $self->{contigs}){
      my $job=();
      push(@{$job->{target}},$self->{prefix}.".disco.fa");
...
sub _def_parameters{
      ...
      #${TIME} -v -o time_Disco_ILL150bp60X.txt Disco-pe  name=HG00733-Disco-ILL60X250
      #np=20 k=128
      #B=40G H=4 kc=3 v=-v contigs 2>Disco60X.err >Disco60X.log
...
sub _create_jobs_short{
    ...
    my @mopt=("name=".$self->{prefix}.".Disco2",
      $params,
      "np=$self->{cores}",
      "lib=\"".join(" ",@lnames)."\"",
      join(" ",@libs),
      "contigs",
      "2>",$self->{prefix}.".Disco2.err",
      ">",$self->{prefix}.".Disco2.log");
      my $job=();
      #the Disco2 contigs
      push(@{$job->{target}},$self->{prefix}.".Disco2-contigs.fa");
      push(@{$job->{cmds}},join(" ",Disco2_BIN,@mopt));
      push(@{$self->{jobs}},$job);
...
sub _create_seqtk_jobs{
  ...
  my $target=$contigs;
  $target=~s/.Disco2-contigs.fa/.Disco2.contigs.fa/;
